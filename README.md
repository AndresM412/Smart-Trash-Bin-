...
1. **Detección de Presencia**: El sensor ultrasónico HC-SR04 monitorea objetos a menos de 30 cm.
2. **Captura de Voz**: El sistema reproduce un saludo (`Bienvenido.mp3`) y graba la petición del usuario (`peticion.wav`).
3. **Procesamiento de Lenguaje (NLP)**:
...
4. **Ejecución Física**: Dependiendo de la clasificación, se activa uno de los dos servomotores para abrir la tapa correspondiente.
...
2. Configura tus credenciales en un archivo `.env`:
   ```env
